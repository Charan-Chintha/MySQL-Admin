delimiter $$
use mysql
drop procedure if exists SP_HOST_Grant$$

create procedure SP_HOST_Grant(in Old_HOST varchar(100),in New_HOST varchar(100))
begin
...
select count(1) into V_cnt from mysql.user where host=Old_host;
 ...
 set @t1 =concat("insert into mysql.user(",V_in_Val,") select ",V_Val," from mysql.user where Host='",Old_HOST,"';");
...
select count(1) into V_cnt from mysql.db where host=Old_host;
 ...
 set @t1 =concat("insert into mysql.db(",V_in_Val,") select ",V_Val," from mysql.db where Host='",Old_HOST,"';");
...
select count(1) into V_cnt from mysql.tables_priv where host=Old_host; 
 ...
 set @t1 =concat("insert into mysql.tables_priv(",V_in_Val,") select ",V_Val," from mysql.tables_priv where Host='",Old_HOST,"';");
...
select count(1) into V_cnt from mysql.columns_priv where host=Old_host; 
 ...
 set @t1 =concat("insert into mysql.columns_priv(",V_in_Val,") select ",V_Val," from mysql.columns_priv where Host='",Old_HOST,"';");
...
select count(1) into V_cnt from mysql.procs_priv where host=Old_host; 
 ...
 set @t1 =concat("insert into mysql.procs_priv(",V_in_Val,") select ",V_Val," from mysql.procs_priv where Host='",Old_HOST,"';");
